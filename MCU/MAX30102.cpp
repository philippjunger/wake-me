...
changed by Lukas Hokenmaier and Philipp Junger
...
static const uint8_t MAX30102_LED1_PULSEAMP = 	0x0D;
...
void MAX30102::setup(byte powerLevel, byte sampleAverage, byte ledMode, int sampleRate, int pulseWidth, int adcRange) {
  ...
  //Mode Configuration
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  if (ledMode == 2) setLEDMode(MAX30102_MODE_REDIRONLY); //Red and IR
  ...
  //if (ledMode > 2) enableSlot(3, SLOT_RED_PILOT);
  //if (ledMode > 3) enableSlot(4, SLOT_IR_PILOT);
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
uint8_t MAX30102::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
  _i2cPort->beginTransmission(address);
  _i2cPort->write(reg);
